...
class ChgpwdScene {
    constructor(options, manager) {
        ...
        this.need_loading.push(["prguse", 81]) // 修改密码：同意
    ...
    update() {
        if (this.need_loading.length > 0) {
            ...
            // 同意按钮
            const bt_chgpwd_ok = globalThis.BaseTextureCache['prguse/81']
    ...
    // 同意按钮
    ok_click() {
        this.manager.send_change_pwd(document.getElementById("chgpwd_id").value
            , document.getElementById("chgpwd_cur_pw").value
            , document.getElementById("chgpwd_new_pw").value)
    }

    on_chgpwd_response(resp) {
        if (resp.ident == SDK.Messages.SM_CHGPASSWD_FAIL) {
            let tip = "输入的新密码长度小于四位！"
            if (resp.recog == -1) {
                tip = "密码错误。不能进行密码变更。"
            } else if (resp.recog == -2) {
                tip = "此帐号被锁定！"
            }
            this.manager.dlg_message(tip)
        } else if (resp.ident == SDK.Messages.SM_CHGPASSWD_SUCCESS) {
            this.manager.dlg_message("密码变更成功.", [SDK.DlgButtons.mbOk], (mb) => {
                this.manager.change_scene(0)
            })
        }
    }
